...
public class AnATLyzer2EFinder implements DialectToIRCompiler {
	...
	private efinder.ir.ocl.OclExpression compileExpression(OclExpression expression, Environment environment) {
		if (expression instanceof StringExp) {
			...
			if (originalIt.getInferredType() == null) {
				// We allow the type to be null because frontends like AnATLyzer may not include the type 
				type = null;
			} else {
				type = getType(originalIt.getInferredType());
			}
				
			efinder.ir.ocl.Iterator iteratorVar = IRBuilder.newIterator(originalIt.getVarName(), type);
			...
			Type varType = varDcl.getInferredType();
			if (varType == null) {
				// Because the let type in ATL is optional
				varType = getTypeFromExpression(varDcl.getInitExpression());
			}
			
			efinder.ir.VariableDeclaration newVarDcl = IRBuilder.newVariableDeclaration(varDcl.getVarName(), getType(varType));			
			...
			if (expression instanceof OclModelElement) {
				OclModelElement me = (OclModelElement) expression;
				Type inferredType = me.getInferredType();
				// This is hack to consider the (unfortunately frequent) case in which AnATLyzer introduces
				// a ThisModule ModelElement but doesn't its inferred type as ThisModuleType.
				if (inferredType == null && "ThisModule".equals(me.getName()))
					return IRBuilder.newModelElement(IRBuilder.newMetaTypeRef(getThisModule()));
				return IRBuilder.newModelElement(getType(inferredType));
			}
			throw new UnsupportedOperationException();
		} else if (expression instanceof OclUndefinedExp) {
	...
	/**
	 * This method is required to "fix" the fact some expressions generated by AnATLyzer's path condition generator
	 * are not typed. Instead of touching AnATLyzer this analyzes expressions by case and tries to get the expected type.
	 */
	private Type getTypeFromExpression(OclExpression expression) {
		Type t = expression.getInferredType();
		if (t != null)
			return t;
		
		if (expression instanceof OperationCallExp) {
			OperationCallExp op = (OperationCallExp) expression;
			if ("oclAsType".equals(op.getOperationName())) {
				t = op.getArguments().get(0).getInferredType();
			}
		}

		if (t == null) {
			throw new UnsupportedOperationException("Can't infer type for " + expression);
		}
	...
	public EFinderModel compile() {
		...
		if (thisModule != null) {
			specification.getTemporary().getClasses().add(thisModule);
			specification.getTemporary().getDangling().add(thisModule.getKlass());
		}
